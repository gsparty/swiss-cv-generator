...
def export_pdf(persona, out_path):
    ...
    pdf.cell(0, 6, f"{persona.occupation} — {persona.canton_name} ({persona.language})", ln=True)
    pdf.ln(4)
    pdf.set_font("Helvetica", size=10)
    pdf.multi_cell(0, 6, persona.summary)
    pdf.ln(4)
    pdf.cell(0, 6, f"Age: {persona.age}    Experience: {persona.years_experience} years", ln=True)
